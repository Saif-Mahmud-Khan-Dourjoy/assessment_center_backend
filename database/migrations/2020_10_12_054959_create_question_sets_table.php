...
class CreateQuestionSetsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('question_sets', function (Blueprint $table) {
            ...
            $table->tinyInteger('status')->default('0')->comment('available or not 0/1, whether the Assessment is published or not, 0 for unpublished where 1 for published');
            ...
            $table->unsignedBigInteger('created_by')->unsigned()->comment('User id');
            $table->foreign('created_by')->references('id')->on('users');
            $table->unsignedBigInteger('updated_by')->unsigned()->comment('user id');
